...
class CalibratedLightningBackend:
    ...
    def load_data(self):
        ...
        self.weather_data = pd.read_csv('weather_impact_backend/Lightning/2024_weather_processed.csv')
         # FIX: Ensure datetime column is properly converted
        self.weather_data['datetime_utc'] = pd.to_datetime(self.weather_data['datetime_utc'])

        print("Loading model...")
        self.model = joblib.load('weather_impact_backend/Lightning/enhanced_enhanced_rf_model.pkl')
        
         # Add this debug line to verify the conversion worked
        print(f"Datetime column type: {self.weather_data['datetime_utc'].dtype}")
        print(f"Loaded {len(self.weather_data)} weather records")

        print("Loading feature list...")
        with open('weather_impact_backend/Lightning/enhanced_6hour_model_results.json', 'r') as f:
            results = json.load(f)
    ...
    def load_training_statistics(self):
        ...
        try:
            training_data = pd.read_csv('weather_impact_backend/Lightning/lightning_6hour_risk_assessment_COMPLETE.csv')
            self.training_stats = {}
    ...
    def predict_lightning_risk(self, features_dict):
        ...
        for feature_name in self.feature_list:
            if feature_name in features_dict and not pd.isna(features_dict[feature_name]):
                ...
                missing_features.append(feature_name)  # Track which features are missing
                missing_count += 1
        
        if missing_count > 0:
            print(f"Missing features: {missing_features}")  # Show which features are missing
...
async def health():
    if backend is None:
        raise HTTPException(status_code=500, detail='Lightning system not initialized')
