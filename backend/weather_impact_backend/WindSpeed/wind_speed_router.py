...
# Initialize the API on import
try:
    CONFIG = {
        'weather_data_path': 'weather_impact_backend/WindSpeed/2024_weather_processed.csv',
        'model_files': {
        'model': 'weather_impact_backend/WindSpeed/wind_power_loss_model_20250907_163805.pkl',
        'scaler': 'weather_impact_backend/WindSpeed/wind_power_loss_scaler_20250907_163805.pkl',
        'encoders': 'weather_impact_backend/WindSpeed/wind_power_loss_encoders_20250907_163805.pkl',
        'config': 'weather_impact_backend/WindSpeed/wind_power_loss_config_20250907_163805.json'
        }
    }
    
    wind_api = AutoWindLossAPI(CONFIG['weather_data_path'], CONFIG['model_files'])
    logger.info("Wind Speed API initialized successfully")
    
except Exception as e:
    logger.error(f"Failed to initialize Wind Speed API: {e}")
    wind_api = None

# FastAPI endpoints (converted from Flask)
@router.get("/health")
...
@router.get("/current")
async def get_current_wind_losses():
...
@router.get("/summary")
async def get_wind_loss_summary():
...
async def test_wind_scenario(request: Dict = None):
    ...
    try:
        ...
        if not ml_result['success']:
            raise HTTPException(status_code=500, detail=ml_result['message'])
...
@router.get("/debug/wind-forecast")
async def debug_wind_forecast():
    ...
    try:
        ...
        if not wind_forecast_result:
            raise HTTPException(status_code=500, detail='No wind forecast data')
...
@router.get("/debug/ml-predictions")
async def debug_ml_predictions():
