...
# Copy all your existing functions exactly (no changes needed)
def get_live_weather_data():
    ...
    return {
        ...
        'Relative_Humidity': 82.0 + np.random.normal(0, 3),
        'Wind_Speed_10m': 6.2 + np.random.normal(0, 0.8),
        'Wind_Direction_50m': 45.0 + np.random.normal(0, 15),
        'Surface_Pressure': 100.9 + np.random.normal(0, 0.2),
        'Wind_Speed_50m': (6.2 + np.random.normal(0, 0.8)) * 1.25,
        'datetime': now,
        'Hour': int(now.hour),
        'Month': int(now.month)
    }
...
def create_physics_features_exact(weather_conditions, turbine_id, label_encoder=None):
    ...
    features_df['Temp_Deviation'] = features_df['Temperature'] - 28.414123
...
def auto_update_worker():
    ...
    while auto_update_enabled:
        try:
            ...
            logger.info(f"Next update in {seconds_to_wait/60:.1f} minutes at {next_hour.strftime('%H:%M')} (2024 simulation)")
            
            # Wait until next hour
            time.sleep(min(seconds_to_wait, 60))  # Check every minute to allow for graceful shutdown
            
            if auto_update_enabled and seconds_to_wait <= 60:
                simulated_time = get_simulated_2024_time()
                logger.info(f"Auto-update triggered at {real_now.strftime('%Y-%m-%d %H:%M:%S')} (simulated: {simulated_time.strftime('%Y-%m-%d %H:%M:%S')})")
        except Exception as e:
            logger.error(f"Error in auto-update worker: {e}")
            time.sleep(60)

# Initialize on import
initialization_success = load_model_and_data()

# FastAPI endpoints
@router.get("/health")
async def health_check():
    """Health check endpoint for consistency with other routers"""
    return {
        "status": "healthy" if (model_package is not None and data is not None) else "unhealthy",
        "service": "Real-Time Turbine Monitoring",
        "model_loaded": model_package is not None,
        "data_loaded": data is not None,
        "simulation_mode": "2024",
...
@router.get("/real-time-turbines")
async def real_time_turbines():
    """Get real-time turbine status with 2024 simulation - 3 time periods"""
    try:
        if model_package is None or data is None:
            raise HTTPException(status_code=503, detail='Model or data not loaded properly')
        ...
        logger.error(f"Error in real_time_turbines: {e}")
        raise HTTPException(status_code=500, detail=f'Internal server error: {str(e)}')

# Alias for backward compatibility
@router.get("/turbines")
async def turbines_alias():
    """Alias for /real-time-turbines for backward compatibility"""
    return await real_time_turbines()
...
async def start_auto_update():
    ...
    try:
        if not auto_update_enabled:
            ...
            logger.info("Auto-update started for real-time turbine monitoring")
        ...
        raise HTTPException(status_code=500, detail=f'Failed to start auto-update: {str(e)}')
...
async def stop_auto_update():
    ...
    try:
        auto_update_enabled = False
        logger.info("Auto-update stopped for real-time turbine monitoring")
        ...
        raise HTTPException(status_code=500, detail=f'Failed to stop auto-update: {str(e)}')