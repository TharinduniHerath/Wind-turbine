...
class WindFarmPipeline:
    ...
    def load_models(self, model_path: str, scaler_path: str, corrections_path: str):
        ...
        try:
            ...
            print(f"Wind corrections loaded: {len(self.wind_corrections)} turbines")
            self.is_loaded = True
            return True
    ...
    def load_weather_data(self, csv_file_path: str):
        ...
        try:
            ...
            print(f"CSV file loaded with {len(self.weather_data)} rows")
    ...
    def _create_hourly_timestamps_2024(self):
        ...
        print(f"Created {len(timestamps)} hourly timestamps starting from {start_date}")
    ...
    def get_weather_forecast(self, hours: int = 48) -> pd.DataFrame:
        ...
        print(f"Current real time: {current_real_time}")
        print(f"Looking for 2024 data at: {parallel_2024_time_rounded}")
        print(f"Found closest 2024 data at: {self.weather_data.loc[closest_index, 'datetime']}")
        ...
        print(f"Generated 2024 forecast from {result_df['datetime'].min()} to {result_df['datetime'].max()}")
    ...
    def calculate_circular_stats(self, wind_dir_series: pd.Series, window: int) -> float:
        ...
        try:
            ...
            print(f"Error in calculate_circular_stats: {e}")
            return 0.0
    ...
    def predict_all_turbines(self, weather_forecast: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"Generated predictions for {len(self.turbine_list)} turbines, {len(weather_forecast)} hours each")
        print(f"Total predictions: {len(predictions_df)}")
        print(f"Power loss range: {predictions_df['predicted_power_loss'].min():.1f} - {predictions_df['predicted_power_loss'].max():.1f} kW")
    ...
    def update_predictions(self):
        ...
        try:
            ...
            print(f"Predictions updated successfully at {current_time.strftime('%H:%M')}")
...
# FastAPI app
app = FastAPI(
    title="Wind Farm Power Loss API - 10 Turbines",
    description="Hourly power loss predictions using trained XGBoost model with 2024 weather data",
    version="4.1.0"
)
...
async def startup_event():
    ...
    try:
        print("Initializing unified wind farm pipeline...")
        ...
        success = pipeline.load_models(
            ...
            corrections_path="turbine_wind_corrections.json"
        )
...
async def root():
    return {
        "message": "Wind Farm Hourly Power Loss Predictions - 10 Turbines (2024 Demo)",
        "status": "online",
        ...
        "demo_mode": "2024 parallel time mapping"
    }
