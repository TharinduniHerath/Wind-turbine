...
# Import turbine_data functions (these exist)
try:
    from turbine_data import get_turbine_health_scores, get_turbine_predictions, generate_dynamic_sensor_data
except ImportError:
    print("⚠️ Warning: turbine_data module not found, some functions may not work")
    # Create dummy functions if imports fail
    def get_turbine_health_scores(turbine_id: str):
        return {"Main Bearing": {"score": 95, "trend": "stable"}}
    
    def get_turbine_predictions(turbine_id: str):
        return {"Gearbox": {"status": "Normal", "message": "Operating normally"}}
    
    def generate_dynamic_sensor_data(turbine_id: str):
        return {"wind_speed": 15.0, "power_output": 2000.0}

# Import ML predictor modules (these exist)
try:
    ...
    print("⚠️ Warning: Some ML predictor modules not found, using fallback functions")
    # Create dummy functions if imports fail
...
def load_models():
    ...
    try:
        ...
        for model_path in possible_paths:
            try:
                if os.path.exists(f"{model_path}random_forest_model.pkl"):
        ...
        try:
            for model_path in possible_paths:
                try:
                    ...
                    print(f"⚠️ Could not load LSTM from {model_path}: {e}")
                    continue
        except Exception as e:
            ...
            for model_path in possible_paths:
                try:
                    if os.path.exists(f"{model_path}scaler.pkl"):
                        ...
                        print(f"✅ Scaler loaded from {model_path}")
                        break
                except Exception as e:
                    print(f"⚠️ Could not load scaler from {model_path}: {e}")
                    continue
        except Exception as e:
        ...
        try:
            for model_path in possible_paths:
                try:
                    feature_path = model_path.replace("models/", "preprocessed_data/feature_names.json")
                    ...
                    print(f"⚠️ Could not load feature names from {feature_path}: {e}")
                    continue
        except Exception as e:
        ...
        if model_loaded:
            ...
            print("⚠️ No models loaded, using fallback predictions")
            return False
        
    except Exception as e:
        print(f"❌ Error in model loading: {e}")
        print("⚠️ Continuing with fallback predictions")
        return False
