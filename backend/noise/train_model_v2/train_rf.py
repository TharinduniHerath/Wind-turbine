...
# # Outputs = turbine responses we want to predict
...
# # --- Sample weights (penalize below cut-in and above cut-out) ---
...
# # --- Save model ---
# joblib.dump(model, os.path.join(MODEL_SAVE_DIR, "rf_model.pkl"))
...
# --- Physics guard ---
def physics_guard(preds, wind_speeds):
    """
    preds: numpy array (n_samples, 3) -> power, RPM, noise
    wind_speeds: numpy array (n_samples,)
    """
    y = preds.copy().astype(float)
    wind_speeds = np.array(wind_speeds).flatten()
    
    stopped = wind_speeds < CUT_IN
    over = wind_speeds >= CUT_OUT
    rated_zone = (wind_speeds >= RATED) & (wind_speeds < CUT_OUT)
    
    y[stopped, 0] = 0.0  # Power
    y[stopped, 1] = 0.0  # RPM
    y[stopped, 2] = 0.0  # Noise
...
# --- Predict & evaluate ---
preds = model.predict(X_test)
preds_guarded = physics_guard(preds, X_test["WindSpeed at 80m"].values)
...
# --- Save model ---
model_path = os.path.join(MODEL_SAVE_DIR, "rf_model_physics_aware.pkl")
