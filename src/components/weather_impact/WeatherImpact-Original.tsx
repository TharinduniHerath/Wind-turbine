...
import WindTurbine from '../Turbine3DModelV2/WindTurbine';
import WindmillTower from '../Turbine3DModelV2/WindmillTower';
import WindmillNacelle from '../Turbine3DModelV2/WindmillNacelle';
import WindmillRotor from '../Turbine3DModelV2/WindmillRotor';
    ...
    const fetchData = async () => {
      try {
        ...
        const [currentRes, summaryRes] = await Promise.all([
          fetch('http://localhost:8000/api/predictions/current'),
          fetch('http://localhost:8000/api/predictions/summary')
        ]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [currentRes, summaryRes] = await Promise.all([
          fetch('http://localhost:5000/api/wind-loss/current'),
          fetch('http://localhost:5000/api/wind-loss/summary')
        ]);
...
const { 
  ...
  const fetchTurbineData = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:5002/api/real-time-turbines');
  ...
  const handleDownload48h = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/download-predictions-csv');
